import React from 'react';
import { useLanguage } from '../context/language-context';
import { Button } from './ui/button';
import { Badge } from './ui/badge';
import { Card } from './ui/card';
import { Check, Tag } from 'lucide-react';
import { TextGenerateEffect } from './ui/text-generate-effect';
import {
  getFadeInBlurAnimation,
  staggerContainerVariants,
  staggerItemVariants,
} from '../lib/animation';
import { motion } from 'framer-motion';

export const Pricing: React.FC = () => {
  const { t, handleOpenLoginModal } = useLanguage();

  return (
    <section
      id="pricing"
      className="py-16 sm:py-24 bg-card overflow-hidden transition-colors duration-200"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mx-auto max-w-2xl text-left sm:text-center space-y-4">
          <motion.div {...getFadeInBlurAnimation(0.1)}>
            <Badge variant="accent">
              <Tag className="size-3.5 mr-0.5" />
              {t.pricing.label}
            </Badge>
          </motion.div>

          <TextGenerateEffect
            tag="h2"
            words={t.pricing.heading}
            className="text-3xl sm:text-4xl lg:text-5xl font-semibold leading-tight text-foreground"
          />

          <motion.p
            {...getFadeInBlurAnimation(0.3)}
            className="text-base text-muted-foreground sm:text-lg"
          >
            {t.pricing.description}
          </motion.p>
        </div>

        <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          <motion.div
            {...getFadeInBlurAnimation(0.2)}
            className="flex h-full"
          >
            <Card className="flex flex-col h-full w-full hover:border-primary transition-all duration-300 hover:-translate-y-1">
              <div className="space-y-6">
                <div className="flex items-center justify-end">
                  <Badge variant="outline">
                    {t.pricing.plans.basic.name}
                  </Badge>
                </div>

                <div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-semibold text-foreground tracking-tight">
                      {t.pricing.plans.basic.price}
                    </span>

                    <span className="text-muted-foreground text-sm">
                      {t.pricing.period}
                    </span>
                  </div>

                  <p className="mt-2 text-muted-foreground text-sm leading-relaxed">
                    {t.pricing.plans.basic.description}
                  </p>
                </div>

                <motion.div
                  variants={staggerContainerVariants(0.08)}
                  initial="hidden"
                  whileInView="visible"
                  viewport={{ once: true }}
                  className="pt-4 border-t border-dashed space-y-3"
                >
                  {t.pricing.plans.basic.features.map((feature, i) => (
                    <motion.div
                      variants={staggerItemVariants}
                      key={i}
                      className="flex items-start gap-3 text-sm text-foreground"
                    >
                      <Check className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                      <span>{feature}</span>
                    </motion.div>
                  ))}
                </motion.div>
              </div>

              <div className="mt-auto pt-8">
                <Button
                  variant="outline"
                  size="md"
                  onClick={handleOpenLoginModal}
                  className="w-full"
                >
                  {t.pricing.plans.basic.cta}
                </Button>
              </div>
            </Card>
          </motion.div>

          <motion.div
            {...getFadeInBlurAnimation(0.3)}
            className="flex h-full"
          >
            <Card className="relative flex flex-col h-full w-full border-2 border-primary rounded-2xl p-8 shadow-2xl shadow-primary/10 lg:-translate-y-2 hover:-translate-y-3 transition-all duration-300">
              <div className="space-y-6">
                <div className="flex items-center justify-end">
                  <Badge variant="accent">
                    {t.pricing.plans.business.name}
                  </Badge>
                </div>

                <div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-semibold text-foreground tracking-tight">
                      {t.pricing.plans.business.price}
                    </span>

                    <span className="text-muted-foreground text-sm">
                      {t.pricing.period}
                    </span>
                  </div>

                  <p className="mt-2 text-muted-foreground text-sm leading-relaxed">
                    {t.pricing.plans.business.description}
                  </p>
                </div>

                <motion.div
                  variants={staggerContainerVariants(0.08)}
                  initial="hidden"
                  whileInView="visible"
                  viewport={{ once: true }}
                  className="pt-4 border-t border-dashed space-y-3"
                >
                  {t.pricing.plans.business.features.map((feature, i) => (
                    <motion.div
                      variants={staggerItemVariants}
                      key={i}
                      className="flex items-start gap-3 text-sm text-foreground"
                    >
                      <Check className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                      <span className="font-medium">{feature}</span>
                    </motion.div>
                  ))}
                </motion.div>
              </div>

              {/* Button pushed to bottom */}
              <div className="mt-auto pt-8">
                <Button
                  variant="primary"
                  size="md"
                  onClick={handleOpenLoginModal}
                  className="w-full"
                >
                  {t.pricing.plans.business.cta}
                </Button>
              </div>
            </Card>
          </motion.div>

          <motion.div
            {...getFadeInBlurAnimation(0.4)}
            className="flex h-full"
          >
            <Card className="flex flex-col h-full w-full hover:border-primary transition-all duration-300 hover:-translate-y-1">
              <div className="space-y-6">
                <div className="flex items-center justify-end">
                  <Badge variant="outline">
                    {t.pricing.plans.entrepreneur.name}
                  </Badge>
                </div>

                <div>
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-semibold text-foreground tracking-tight">
                      {t.pricing.plans.entrepreneur.price}
                    </span>

                    <span className="text-muted-foreground text-sm">
                      {t.pricing.period}
                    </span>
                  </div>

                  <p className="mt-2 text-muted-foreground text-sm leading-relaxed">
                    {t.pricing.plans.entrepreneur.description}
                  </p>
                </div>

                <motion.div
                  variants={staggerContainerVariants(0.08)}
                  initial="hidden"
                  whileInView="visible"
                  viewport={{ once: true }}
                  className="pt-4 border-t border-dashed space-y-3"
                >
                  {t.pricing.plans.entrepreneur.features.map((feature, i) => (
                    <motion.div
                      variants={staggerItemVariants}
                      key={i}
                      className="flex items-start gap-3 text-sm text-foreground"
                    >
                      <Check className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                      <span>{feature}</span>
                    </motion.div>
                  ))}
                </motion.div>
              </div>

              <div className="mt-auto pt-8">
                <Button
                  variant="outline"
                  size="md"
                  onClick={handleOpenLoginModal}
                  className="w-full"
                >
                  {t.pricing.plans.entrepreneur.cta}
                </Button>
              </div>
            </Card>
          </motion.div>
        </div>
      </div>
    </section>
  );
};
