...
import { X, Eye, EyeOff, Layers, Mail, Lock, CheckCircle, AlertCircle } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground flex items-center gap-1.5">
              ...
              <span>{t.loginModal.passwordLabel}</span>
            </label>
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-2.5 pr-10 bg-input text-foreground border ${errors.password ? 'border-destructive focus:ring-destructive' : 'border-border focus:border-primary'
                  } rounded-xl text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring transition-all`}
                {...register('password')}
              />
              <button
                type="button"
